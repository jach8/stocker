...
class data:
	...
	def _get_test_data(self, stock):
		...
		if 'target' in mdf.columns:
			mdf = mdf.drop(columns = ['target'])

		mdf.insert(0, 'stock', stock)
...
if __name__ == "__main__":
	...
	df = pd.concat([d._get_test_data(stock) for stock in tqdm(d.stocks['all_stocks'])])
